...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (LastRead && lastType === script.dataset.type) {
        ...
        reada.href = `./read/chapter-${LastRead + 1}`;
...
function displayChapters() {
    ...
    ChapterList.forEach(chapter => {
        chSearchresult.push(`<div class="chapter_item"><a href="./read/chapter-${chapter.index + 1}"><p>${chapter.title}</p></a></div>`);
    });
...
function findChapter(value) {
    ...
    for (let i = 0; i < ChapterList.length; i++) {
        ...
        if (chSearchindex !== -1) {
            chSearchresult.push(`<div class="chapter_item"><a href="./read/chapter-${index + 1}"><p>${displayTitle}</p></a></div>`);
        }
    }
    chapterSearch.innerHTML = chSearchresult.join("");
